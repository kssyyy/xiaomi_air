...
from miio import AirConditioningCompanion, Device, DeviceException
...
class ACControllerApp:
    ...
    def _get_plug_client(self, row: DeviceRow):
        if row.plug_client is None:
            row.plug_client = Device(ip=row.config.ac_ip, token=row.config.token)
        return row.plug_client

    def _plug_send(self, row: DeviceRow, method: str, params):
        self.log(f"{row.config.name} PLUG CMD: {method} {params}")
        try:
            result = self._get_plug_client(row).send(method, params)
            self.log(f"{row.config.name} PLUG RET: {result}")
            return result
        except DeviceException as exc:
            row.plug_client = None
            self.log(f"{row.config.name} PLUG ERR: {exc}")
            return None
        except Exception as exc:
            self.log(f"{row.config.name} PLUG ERR: {exc}")
            return None

    def _plug_action_ok(self, value):
        if isinstance(value, dict):
            return value.get("code") == 0
    ...
    def _toggle_plug(self, row: DeviceRow):
        payloads = [
            {"did": "call-2-1", "siid": 2, "aiid": 1, "in": []},
            {"did": "2-1", "siid": 2, "aiid": 1, "in": []},
        ]
        for payload in payloads:
            res = self._plug_send(row, "action", payload)
            if res is not None and self._plug_action_ok(res):
                return True
        self.log(f"{row.config.name}: 插座toggle(2-1)失败")
        return False

    def _get_plug_state(self, row: DeviceRow):
        res = self._plug_send(
            row,
            "get_properties",
        ...
        if res is None:
            self.log(f"{row.config.name}: 插座读取状态失败")
            return None
        if not isinstance(res, list) or not res or not isinstance(res[0], dict):
            self.log(f"{row.config.name}: 插座状态返回异常: {res}")
            return None
        item = res[0]
        if item.get("code") not in (0, None):
            self.log(f"{row.config.name}: 插座读取状态失败: {res}")
            return None
        ...
        if isinstance(value, int):
            return value != 0
        self.log(f"{row.config.name}: 插座状态解析失败")
        return None

    def send_ac(self, row: DeviceRow, action: str):
        if row.config.command == PLUG_COMMAND:
            target_is_on = action == "on"
            state = self._get_plug_state(row)
            if state is None:
                self.log(f"{row.config.name}: 插座{STATE_LABEL_MAP.get(action, action)}失败")
                return
            if state == target_is_on:
                ...
                self.log(f"{row.config.name}: 插座已是{STATE_LABEL_MAP.get(action, action)}，无需切换")
                return
            ok = self._toggle_plug(row)
            if not ok:
                self.log(f"{row.config.name}: 插座第1次发送失败")
                return
            self.log(f"{row.config.name}: 插座第1次发送成功")
            time.sleep(DOUBLE_SEND_GAP_SECONDS)
            state_after_first = self._get_plug_state(row)
            if state_after_first is None:
                self.log(f"{row.config.name}: 插座第2次前状态读取失败，跳过第2次")
            elif state_after_first != target_is_on:
                ok2 = self._toggle_plug(row)
                if ok2:
                    self.log(f"{row.config.name}: 插座第2次发送成功")
                else:
                    self.log(f"{row.config.name}: 插座第2次发送失败")
            else:
                self.log(f"{row.config.name}: 插座第2次无需发送")

            final_state = self._get_plug_state(row)
            if final_state == target_is_on:
                row.last_sent_action = action
                row.last_send_ts = time.time()
                self.ui_queue.put(("update", row, {"state": STATE_LABEL_MAP.get(action, action)}))
                self.log(f"{row.config.name}: 插座 -> {STATE_LABEL_MAP.get(action, action)}")
            else:
                self.log(f"{row.config.name}: 插座{STATE_LABEL_MAP.get(action, action)}失败")
            return
    ...
    def apply_mode_temp_worker(self, row: DeviceRow, mode: str, temp: int):
        if row.config.command == PLUG_COMMAND:
            self.log(f"{row.config.name}: 当前设备({PLUG_COMMAND})不支持模式/温度设置")
            return
        if mode not in MODE_VALUE_TO_LABEL:
...
def main():
    ...
    app.log(
        f"已加载 {len(configs)} 台设备。轮询间隔={app.interval_seconds}秒, 批量={MAX_DEVICES_PER_BATCH}"
    )
    app.log("通信模式: python-miio (no miiocli dependency)")
    root.mainloop()
